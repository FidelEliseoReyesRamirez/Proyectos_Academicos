...
import {
    ArrowLeft, Save, Briefcase, Calendar, User as UserIcon,
    Tag, AlignLeft, Search, X, Check, ChevronDown, Layers, BookOpen, Users, Activity,
} from 'lucide-react';
...
// ============================================================
// Multiseleccion con chips para areas tematicas.
// ============================================================
...
export default function ProyectosEdit({
    proyecto, periodos, estudiantes, tutores, revisores, revisoresActuales,
}: Props) {
    const areasIniciales = proyecto.area_tematica
        ? proyecto.area_tematica.split(',').map(s => s.trim()).filter(Boolean)
        : [];
    ...
    const form = useForm<{
        ...
        area_tematica: areasIniciales,
        estado: proyecto.estado,
        periodo_id: String(proyecto.periodo_id),
        estudiante_id: String(proyecto.estudiante_id),
        tutor_id: String(proyecto.tutor_id),
        revisores_ids: revisoresActuales.map(String),
    });

    const submit = (e: FormEvent) => {
        e.preventDefault();
        form.put(`/proyectos/${proyecto.id}`, { preserveScroll: true });
    };

    return (
        <>
            <Head title={`Editar ${proyecto.codigo}`} />
            ...
            <style>{`
                ...
                @media (prefers-color-scheme: dark) {
                    .codigo-badge { background: rgba(214,185,106,0.18); color: #F4EEE9; }
                }
                    ...
                    <section className="users-card">
                        <div className="users-card-header">
                            <div className="users-card-header-inner">
                                ...
                                <h1 className="users-title">Editar Proyecto Academico</h1>
                                <div className="codigo-badge">
                                    <Tag className="h-3 w-3" />
                                    {proyecto.codigo}
                                </div>
                                <p className="users-description">
                                    Modifica los datos del proyecto. Los cambios de estado quedan registrados
                        ...
                        <form onSubmit={submit} className="grid gap-4 p-5 md:grid-cols-2">
                            ...
                            <div>
                                <label className="field-label">
                                    <Activity className="h-3 w-3" /> Estado del proyecto
                                </label>
                                <SearchableCombobox
                                    options={ESTADOS.map(e => ({ id: e.value, label: e.label }))}
                                    value={form.data.estado}
                                    onChange={v => form.setData('estado', v)}
                                    placeholder="Selecciona estado"
                                    emptyText="Sin coincidencias"
                                />
                                {form.errors.estado && <div className="error-text">{form.errors.estado}</div>}
                            </div>
